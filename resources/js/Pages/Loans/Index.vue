                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive p-0">
                            <Table v-if="tableData" :loans ="tableData" />
                            <Table v-else :loans ="loans" />
                        </div>
                    </div>
                </div>
            </div>
...
<script setup>
    ...
    const tableData = computed(()=> {
        const name_sort = props.loans.filter( (loan) => loan.name.toLowerCase().includes(user.value.toLowerCase()));
        if(!status.value){
            return name_sort;
        }
    ...
    const SendDate = () => {
        if(date.value){
            router.get(`/loans?name=${user.value}&status=${status.value}&date=${date.value}`)
        }
    }
</script>
